...
if (process.env.generate === true) {
    ...
    var build = childProcess.fork(
        path.join(__dirname, "..", "utils", "build-commands.js")
    ); // run build-css.js
...
const {
    ButtonInteraction,
    CommandInteraction,
} = require("./handlers/Interactions.js"); // Imports handlers for interaction types

// Create client instance and set the intents and status
const client = new Client({
    intents: [Intents.FLAGS.GUILDS],
    permission: [Intents.FLAGS.MANAGE_ROLES],
    presence: {
        status: "online",
        afk: false,
        activities: [
...
client.on("interactionCreate", async interaction => {
    ...
    return interaction.reply({
        content: "Interaction type invalid",
        ephemeral: true,
    });
});
